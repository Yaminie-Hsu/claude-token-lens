...
> 让 Claude Code 的 context window 用得更久 — 实时监控用量、压缩输入、让每次会话走得更远
...
浪费 Token 的真正代价不是钱——是 **context window 的寿命**。每一条臃肿的 Prompt 都在加速把早期对话内容挤出窗口，迫使你更频繁地 `/compact` 或 `/clear`，辛苦积累的上下文就此丢失。
...
Claude Code 已经提供了 `/compact`、`/clear`、自动压缩和 Prompt 缓存。`claude-token-lens` 负责更早的环节——在 Prompt 提交之前就完成处理，让那些官方工具面对的内容更少。输入压力越小 → 强制压缩频率越低 → 上下文保留得越久。
...
- **`PreToolUse` hook** — 过滤 Bash 工具输出：
  ...
  - 日志/文件读取 → 截取最后 N 行
- **会话统计** — `token-lens stats` 查看最近 30 天汇总；`token-lens stats session` 查看当前会话
- **月度账单追踪** — `token-lens stats month` 显示本月所有会话的 API 等效费用合计，与 Claude Pro 月费（$20）对比，直观判断订阅是否划算
- **实时状态栏** — 在 Claude Code 状态栏嵌入当前会话数据：`lens  8.4k↑ 2.1k↓ · ctx 15% · usage 20% · Resets in 3hr 23min · $0.0535 · saved 1.2k (14%)`
- **上下文用量指示** — 状态栏超 50% 变黄，超 80% 变红并提示 `/compact or /clear`
...
### stats session（当前会话视图）

`token-lens stats session` 只展示本次 Claude Code 会话的数据，含逐条 Prompt 记录表格：

```
── token-lens  current session ──────────────────
  Session ID         : a1b2c3d4…  (started 2026-04-24 10:23:01)
  Prompts sent       : 12
  Tokens submitted   : 8,412
  Tokens after lens  : 7,234
  Tokens saved       : 1,178  (avg 14.0% per prompt)
  Est. cost saved    : $0.0035  (Sonnet output rate)

  Time                 Before    After   Saved  Strategies
  ───────────────────  ───────  ───────  ───────  ────────────────────
  2026-04-24 10:24:05      562      517       45  stack-trace, whitespace
  ...
```

### 实时状态栏

`token-lens setup` 同时在 `~/.claude/settings.json` 中注册 `statusLine` 脚本。每次 Claude 回复后，状态栏自动刷新：

```
lens  8.4k↑ 2.1k↓ · ctx 15% · usage 20% · Resets in 3hr 23min · $0.0535 · saved 1.2k (14%)
```

- `↑` 输入 token，`↓` 输出 token（本次会话累计）
- `ctx %` — context window 占用率（200k）；超过 50% 变黄，超过 80% 变红并提示 `/compact or /clear`
- `usage %` — 5小时速率限制消耗量，与官方 Claude Code "Current Session" 进度条数据一致
- `Resets in` — 速率限制窗口剩余重置时间
- `$` — 本次会话费用（美元），每次刷新时自动写入本地数据库，用于月度统计
- `saved` — 仅在压缩生效时显示
...
每次重启 Claude Code 后计数归零。费用数据会跨会话自动累计到本地 SQLite，无需任何手动操作。
...
"API-equivalent cost" 是按 API 按量计费模式下的等效费用。将其与 Pro 月费 $20 对比，即可判断订阅是否回本。也可指定月份：
...
# 查看当前会话统计（每次重启 Claude Code 重置）
token-lens stats session

# 查看本月 API 等效费用 vs Claude Pro 月费
token-lens stats month
token-lens stats month 2026-03
